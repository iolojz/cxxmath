...
struct less_manifold_indices {
	...
	apply( const std::variant<int, std::string_view> &i1, const std::variant<int, std::string_view> &i2 ) {
	...
	struct extract_indices {
		...
		static auto apply( T &&t ) {
			if constexpr( std::is_same_v<cxxmath::tag_of_t<T>, cxxmath::tag_of_t<formal_metric_entry>> ) {
				...
				return std::visit(
					[]( auto &&r ) { return apply( std::forward<decltype( r )>( r ) ); }, std::forward<T>( t )
				);
			} else {
...
using metric_entry_algebra = cxxmath::quotient_r_algebra<
	cxxmath::tag_of_t<free_metric_entry_algebra>, metric_entry_quotient_spec
>;
...
using clifford = cxxmath::quotient_r_algebra<
	cxxmath::tag_of_t<type_helpers::free_gamma_algebra>, type_helpers::clifford_quotient_spec
>;

template<>
class formal_metric_index_handler::contract_indices<cxxmath::tag_of_t<type_helpers::free_metric_entry_algebra>> {
			...
			if( std::size( indices ) == 1 ) {
				...
				if( fme1.index1 == fme2.index1 ) {
					...
					throw std::runtime_error(
						"cross-contraction(1) of formal metric entries with mismatching indices"
					);
				}
				...
				return std::make_optional(
					cxxmath::make<free_metric_entry_algebra_tag>(
						std::move( coefficient_one ), std::move( metric_entries )
					)
				);
			} else if( std::size( indices ) == 2 ) {
				if( !( fme1.index1 == fme2.index1 && fme1.index2 == fme2.index2 )
					&& !( fme1.index1 == fme2.index2 && fme1.index2 == fme2.index1 ) ) {
					throw std::runtime_error(
						"cross-contraction(2) of formal metric entries with mismatching indices"
					);
				}
				
				auto result_coefficient =
...
template<> class gamma_index_handler::contract_indices<cxxmath::tag_of_t<type_helpers::free_gamma_algebra>> {
	template<class Range> static decltype( auto ) compose_coefficient( Range &&r ) {
		return cxxmath::detail::coefficient_composer<cxxmath::tag_of_t<type_helpers::metric_entry_algebra>>::apply(
			std::forward<Range>( r )
		);
	}
	
	template<class FME, class Range, class GM, class IndexRange>
	static type_helpers::free_gamma_algebra
	apply_impl( const FME &fme1, const Range &parts, const GM &gm2, const IndexRange &indices ) {
		if constexpr( std::is_same_v<std::decay_t<FME>, formal_metric_entry>
			&& std::is_same_v<std::decay_t<GM>, gamma_matrix> ) {
			...
			auto coefficient =
				compose_coefficient( boost::make_iterator_range( std::begin( parts ), std::end( parts ) ) );
			return cxxmath::make<cxxmath::tag_of_t<type_helpers::free_gamma_algebra>>(
				std::move( coefficient ), std::move( resulting_gamma )
			);
		} else {
...
public:
	...
	static auto
	apply( const IndexedObject1 &io1, const Range &parts, const IndexedObject2 &io2, const IndexRange &indices ) {
...
BOOST_AUTO_TEST_CASE( clifford_algebra_int ) {
	...
	const auto delta_mu_nu = make<clifford_tag>(
		cxxmath::make<cxxmath::tag_of_t<type_helpers::metric_entry_algebra>>(

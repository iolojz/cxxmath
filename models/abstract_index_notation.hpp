...
class coefficient_composer<free_r_algebra_tag<Coefficient, Symbol, CoefficientSet, CoefficientRing, SymbolTotalOrder>> {
	...
	template<class S> static Symbol extract_symbol( S &&part ) {
		if constexpr( is_std_variant_v<std::decay_t<S>> ) {
			return std::visit(
				[]( auto &&p ) { return extract_symbol( std::forward<decltype( p )>( p ) ); }, std::forward<S>( part )
			);
...
class abstract_index_quotient_spec {
	...
	static auto index_intersection( SortedIndexRange1 &&si1, SortedIndexRange2 &&si2 ) {
		using index_type = std::common_type_t<
			typename std::decay_t<SortedIndexRange1>::value_type, typename std::decay_t<SortedIndexRange2>::value_type
		>;
	...
	static auto find_index_coincidence( IteratorPair begin, Range1 &&range1, Range2 &&range2 ) {
		...
		using common_indices_type = std::decay_t<
			decltype( index_intersection(
		...
		do {
			...
			advance_iterator_pair<disjoint>( begin, std::forward<Range1>( range1 ), std::forward<Range2>( range2 ) )
				!= end );
	...
	static std::optional<FRA> perform_ss_contractions( SymbolRange &&symbols ) {
		...
		for( auto current = begin; current.first != end.first; advance( current ) ) {
			...
			if( s1_it == s2_it ) {
				contracted = IndexHandler::template contract_indices<tag_of_t<FRA>>::apply(
					*s1_it, std::move( common_indices )
				);
			} else {
				auto middle = boost::make_iterator_range( std::next( s1_it ), s2_it );
				contracted = IndexHandler::template contract_indices<tag_of_t<FRA>>::apply(
					*s1_it, std::move( middle ), *s2_it, std::move( common_indices )
				);
			}
			...
			auto head = make<tag_of_t<FRA>>(
				FRA::coefficient_ring::one(),
				boost::make_iterator_range( std::begin( symbols ), s1_it )
			);
			auto tail = make<tag_of_t<FRA>>(
				FRA::coefficient_ring::one(),
	...
	static std::optional<FRA> perform_cparts_contractions( CPartRange &&cparts, SymbolRange &&symbols ) {
		...
		for( auto current = begin; current.first != std::end( cparts ); advance( current ) ) {
			...
			auto contracted = IndexHandler::template contract_indices<tag_of_t<FRA>>::apply(
				*cpart_it, std::move( middle ), *s_it, std::move( common_indices )
			);
			if( contracted == std::nullopt ) {
				continue;
			}
	...
	static std::optional<FRA> perform_cs_contractions( Coefficient &&c, SymbolRange &&symbols ) {
		...
		auto contraction_it = std::find_if(
			std::begin( c_decomposed ), std::end( c_decomposed ),
			[&]( auto &&cpart_range ) {
		...
		auto monomial_from_cpart = [&]( auto &&cpart_range ) {
			return monomial{
				symbol_copies, detail::coefficient_composer<tag_of_t<typename FRA::coefficient>>::apply(
					boost::make_iterator_range(
						std::make_move_iterator( std::begin( cpart_range ) ),
		...
		std::transform(
			std::begin( c_decomposed ), contraction_it,
			std::back_inserter( new_monomials ), monomial_from_cpart
		);
		
		{
			const auto &contracted_monomials = contraction_result->monomials();
			new_monomials.insert(
				new_monomials.end(), std::make_move_iterator( std::begin( contracted_monomials ) ),
		...
		return make<tag_of_t<FRA>>(
			std::make_move_iterator( new_monomials.begin() ), std::make_move_iterator( new_monomials.end() )
		);
	}
public:
	...
	struct quotient_map_in_place {
		...
		static FRA &apply( FRA &fra ) {
			static_assert(
				is_std_variant_v<typename tag_of_t<FRA>::symbol> == false,
				"Abstract index notation cannot work with std::variants"
			);
			static_assert(
				is_std_variant_v<typename tag_of_t<FRA>::coefficient> == false,
				"Abstract index notation cannot work with std::variants"
			);
			...
			for( auto monomial = fra.monomials().begin(); monomial != fra.monomials().end(); ++monomial ) {
				...
				if( contraction_result ) {
					...
					const auto &contracted_monomials = contraction_result->monomials();
					new_monomials.insert(
						new_monomials.end(), std::make_move_iterator( contracted_monomials.begin() ),
						std::make_move_iterator( contracted_monomials.end() )
					);
				} else {
			...
			fra = make<tag_of_t<FRA>>(
				std::make_move_iterator( new_monomials.begin() ), std::make_move_iterator( new_monomials.end() )
			);
			new_monomials.clear();
			
			for( auto monomial = fra.monomials().begin(); monomial != fra.monomials().end(); ++monomial ) {
				auto contraction_result =
					perform_cs_contractions<FRA>( std::move( monomial->second ), std::move( monomial->first ) );

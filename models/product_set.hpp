...
struct equal {
	...
	static constexpr auto apply( Product1 &&p1, Product2 &&p2 ) {
		...
		return and_(
			first_set::equal(
				Product::first( std::forward<Product1>( p1 ) ), Product::first( std::forward<Product2>( p2 ) )
			),
			second_set::equal(
...
public:
	static constexpr bool value = std::conditional_t<
		...
		std::bool_constant<has_default_set_v < typename product::unique_first_tag>
			&& has_default_set_v < typename product::unique_first_tag>>
	,
	std::false_type
	>::value;
};
